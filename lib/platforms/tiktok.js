...
export async function uploadVideo(accessToken, videoPath, metadata, onProgress) {
  ...
  let initResponse = await fetch('https://open.tiktokapis.com/v2/post/publish/video/init/', {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${accessToken}`,
  ...
  // If the app is in Sandbox/Unaudited mode, public upload fails with a guidelines/privacy error.
  // We automatically retry with SELF_ONLY (Private/Only Me) privacy level so the developer can still test.
  if (initData.error && initData.error.code !== 'ok') {
    const errorMsg = initData.error.message || '';
    if (metadata.privacyLevel !== 'SELF_ONLY' && (errorMsg.includes('guidelines') || errorMsg.includes('privacy') || errorMsg.includes('unaudited') || errorMsg.includes('sharing'))) {
      console.log('TikTok client is unaudited/sandbox. Retrying upload initialization with SELF_ONLY privacy level.');
      initResponse = await fetch('https://open.tiktokapis.com/v2/post/publish/video/init/', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Content-Type': 'application/json; charset=UTF-8',
        ...
        body: JSON.stringify({
          post_info: {
            title: metadata.title || '',
            privacy_level: 'SELF_ONLY', // Retry as private
            disable_comment: metadata.disableComment || false,
            disable_duet: metadata.disableDuet || false,
            disable_stitch: metadata.disableStitch || false,
          },
