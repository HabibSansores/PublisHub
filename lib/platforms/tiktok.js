...
export function getAuthUrl(clientKey, redirectUri) {
...
export async function getTokensFromCode(clientKey, clientSecret, code, redirectUri) {
...
export async function getUserDetails(accessToken) {
  ...
  const data = await response.json();
  if (data.error) {
    throw new Error(data.error.message || 'Error al obtener datos de TikTok.');
  }
  ...
  throw new Error('No se encontró información del usuario en TikTok.');
}
